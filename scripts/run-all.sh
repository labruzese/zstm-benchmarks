#!/usr/bin/env bash
#
# End-to-end: install/locate Zig, build both sides, run the matrix, emit tables.
#
...
#
# Set ZIG=/path/to/zig to use an existing Zig 0.16+ instead of fetching one.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

if [ -z "${ZIG:-}" ] && [ ! -x toolchain/zig ] && ! command -v zig > /dev/null; then
    echo "==> fetching Zig"
    toolchain/get-zig.sh
fi
...
echo "==> running matrix (nothing else should be running on this machine)"
